...
for fold, (train_index, val_index) in enumerate(skf.split(D_a_balanced, y_a_balanced), 1):
    ...
    plt.title(f"t-SNE des données (pli {fold}) avec pseudo-labels KNN")
    plt.savefig(f"tsne_pseudo_labels_fold_{fold}_KNN.png", dpi=300)
    plt.close()
    ...
    plt.savefig(f"tsne_pseudo_labels_fold_{fold}_filtered_KNN.png", dpi=300)
    plt.close()
    ...
    plt.savefig(f"tsne_labeled_data_KNN{fold}.png", dpi=300)
    plt.close()
