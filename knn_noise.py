...
# Fonction pour ajouter du bruit à toutes les classes
def add_noise(data, labels, noise_level=0.1):
    noisy_data = data.copy()
    noise = noise_level * np.random.randn(*noisy_data.shape)
    noisy_data += noise
    return noisy_data

# Ajout de bruit à toutes les classes avant entraînement
D_a_augmented = add_noise(D_a, y_a, noise_level=0.1)
...
for fold, (train_index, val_index) in enumerate(skf.split(D_a_augmented, y_a), 1):
    D_a_train, D_a_val = D_a_augmented[train_index], D_a_augmented[val_index]
...
plt.tight_layout()
plt.savefig('cv_conf_matrices_knn_noise_all_before.png', dpi=300)
plt.close()
...
plt.savefig("Visualisations_noise_knn_all_before.png", dpi=300)
plt.close()
